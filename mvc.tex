...
First find two parallel planes which contains each line, and find the distance between the parallel planes. This is the distance between lines.
...
To show a limit does not exist at a given point, show that approaching the point from two different paths produces different values. This can be done by setting $y$ to a function of $x$, or vice versa.
...
Clairaut's theorem states that if a function is defined on a point $(a,b)$ and its second partial derivatives $f_{xy}$ and $f_{yx}$ are both continuous, then $f_{xy}(a,b)=f_{yx}(a,b)$.

\subsection{Tangent Plane and Normal Line}
...
The normal line at a given point is the line orthogonal to the tangent plane and passing through said point. The normal line to a surface defined by $F(x,y,z)=0$ at $(x_0,y_0,z_0)$ is:
\begin{align*}
	x&=x_0+F_x(x_0,y_0,z_0)t \\
	y&=y_0+F_y(x_0,y_0,z_0)t \\
	z&=z_0+F_z(x_0,y_0,z_0)t
\end{align*}
To calculate the tangent plane to $F(x,y,z)$ at $(x_0,y_0,z_0)$, first observe that the gradient:
\begin{equation*}
    \nabla F(x_0,y_0,z_0)=(F_x(x_0,y_0,z_0),F_y(x_0,y_0,z_0),F_z(x_0,y_0,z_0))
\end{equation*}
is parallel to the normal line, which is orthogonal to the tangent plane. Therefore, the tangent plane is given by:
\begin{equation*}
	(x-x_0,y-y_0,z-z_0)\cdot \nabla F=0
\end{equation*}
To calculate the normal line to a surface given by $z=f(x,y)$, express it as:
\begin{equation*}
    F(x,y,z)=f(x,y)-z=0
\end{equation*}
Notice that $F_z(x,y,z)=-1$. The normal line to this surface at $(x_0,y_0,z_0)$ where $z_0=f(x_0,y_0)$ is:
\begin{alignat*}{2}
	x&=x_0+F_x(x_0,y_0,z_0)t&&=x_0+f_x(x_0,y_0)t \\
	y&=y_0+F_y(x_0,y_0,z_0)t&&=y_0+f_y(x_0,y_0)t \\
	z&=z_0+F_z(x_0,y_0,z_0)t&&=z_0-t
\end{alignat*}

\subsection{Directional Derivatives}

The directional derivative is a measure of the rate of change of a function along a certain direction vector.

The directional derivative of a function $f:\mathbb R^n\to\mathbb R$ with respect to a unit vector $\mathbf u\in\mathbb R^n$ is:
\begin{equation*}
    D_\mathbf u f(\mathbf x)=\lim_{h\to0}\frac{f(\mathbf x+\mathbf u)-f(\mathbf x)}{h}
\end{equation*}
If $\mathbf u=(u_1,u_2,\ldots,u_n)$ and $\lVert \mathbf u \rVert =1$, the directional derivative with respect to $\mathbf u$ is:
\begin{equation*}
	D_\mathbf u f(\mathbf x)
	=\sum_{i=1}^{n} \diffp{f}{x_i}u_i
	=\nabla f(\mathbf x)\cdot \mathbf u
\end{equation*}
where $\nabla f$ is the gradient. For $f:\mathbb R^2\to\mathbb R$ and $\mathbf u=(a,b)$:
\begin{equation*}
	D_\mathbf u f(x,y)
	=a\diffp fx+b\diffp fy
	=af_x(x,y)+bf_y(x,y)
\end{equation*}
...
\begin{equation*}
	...
	=\left( \diffp{f}{x_1},\diffp{f}{x_2},\ldots,\diffp{f}{x_n} \right)
\end{equation*}
In two dimensions:
\begin{equation*}
    \nabla f(x,y)=(f_x(x,y),f_y(x,y))
\end{equation*}
The gradient at a point is the direction of greatest change at that point. That is, if $\mathbf u\in\mathbb R^n$, $D_\mathbf u f$ is greatest when $\mathbf u$ has the same direction as $\nabla f(\mathbf x)$.

For some value $c\in\mathbb R$, the level curve of value $c$ is the curve defined by $f(\mathbf x)=c$. At some point $\mathbf x_1$, the gradient at that point $\nabla f(\mathbf x_1)$ is always perpendicular to the level curve of value $f(\mathbf x_1)$ when graphed on the same plane.

\subsection{Relative Extrema}

$\mathbf x_1$ is a critical point of $f:\mathbb R^n\to\mathbb R$ if $\nabla f(\mathbf x_1)=0$ or it does not exist.

To determine if $\mathbf x_1$ is a local minimum, local maximum or a saddle point, calculate the Hessian matrix:
\begin{equation*}
    H=\begin{bmatrix}
		f_{xx}(\mathbf x_1) &
		f_{xy}(\mathbf x_1) \\
...
\begin{itemize}
	\item If $\det H>0$, $\mathbf x_1$ is a local minimum if $f_{xx}(\mathbf x_1)>0$ and a local maximum if $f_{xx}(\mathbf x_1)<0$.
	\item If $\det H<0$, $\mathbf x_1$ is a saddle point.
	\item If $\det H=0$, the test is inconclusive.
\end{itemize}
A saddle point is one in which some paths passing through the point results in a local minimum, and some other paths result in a local maximum.

\subsection{Chain Rule}

For one independent variable: if $x=g(t),y=g(t)$ and $z=f(x,y)=f(g(t),h(t))$:
\begin{equation*}
    \diff zt=\diffp zx\diff xt+\diffp zy\diff yt
\end{equation*}
For two independent variables: if $x=g(u,v),y=h(u,v)$ and $z=f(x,y)=f(g(u,v),h(u,v))$:
\begin{equation*}
    \diffp zu=\diffp zx\diffp xu+\diffp zy\diffp yu 
	\qquad\qquad
    \diffp zv=\diffp zx\diffp xv+\diffp zy\diffp yv
\end{equation*}

\subsection{Lagrange Multipliers}

We can find the absolute/relative extrema inside a boundary using by identifying points where $\nabla f=0$. Lagrange Multipliers will find relative extrema on the boundary.

Let $f,g:\mathbb R^n\to\mathbb R$ and $k\in\mathbb R$. We find the relative extrema of $f$ on the boundary $g(\mathbf x)=k$. Potential optimal points $\mathbb x_1$ on the boundary are where the gradients of $f$ and $g$ are scalar multiples, that is:
\begin{align*}
	\nabla f(\mathbf x_1)&=\lambda \nabla g(\mathbf x_1) \\
	g(\mathbf x_1)&=k
\end{align*}
where $\lambda\in\mathbb R$, and is called the Lagrange Multiplier. Then, absolute minimum/maximum on the boundary can be found by comparing the values of $f(\mathbf x_1)$ for all candidate points.

To find the absolute minimum/maximum of a function within a boundary, find candidate points on the boundary with Lagrange Multipliers, and candidate points inside the boundary where $\nabla f(\mathbf x_1)=0$, and compare.
...
\end{equation*}
represents the signed volume of the solid formed between the region $R$ at $z=0$ and the surface $z=f(x,y)$. $R$ represents any region, including but nor limited to rectangles.

\subsection{Fubini's Theorem}

Let $R$ be a rectangular region defined by:
\begin{equation*}
    a\leq x\leq b, c\leq y\leq b
\end{equation*}
If $f$ is continuous on $R$, then the bounds of the double integral can be switched.
\begin{equation*}
    \iint_R f(x,y) \,\mathrm{d}A
...
The average value of a function on a region $R$ is, where $A_R$ is the area of $R$:
\begin{equation*}
	\text{Average}=\frac{1}{A_R}\iint_R f(x,y) \,\mathrm{d}A
\end{equation*}

\subsection{Separable Multiple Integrals}

On a rectangular region $[a,b]\times[c,d]$ (meaning $a\leq x\leq b$ and $c\leq y\leq d$), any integral whose integrand can be expressed as a product of single variable functions equals:
\begin{equation*}
	\int_{a}^{b}\int_{c}^{d}f(x)g(y)\,\mathrm{d}y\,\mathrm{d}x
	=\int_{a}^{b}f(x)\,\mathrm{d}x \int_{c}^{d}g(y)\,\mathrm{d}y
\end{equation*}
This can be generalized for any number of integrals.

\subsection{Surface Area}

The area of the surface $z=f(x,y)$ over a region $R$ where both partial derivatives $f_x,f_y$ are continuous is:
\begin{equation*}
    S=\iint_R \sqrt{1+f_x(x,y)^2+f_y(x,y)^2}\,\mathrm{d}A
\end{equation*}

\subsection{Polar Coordinates}

A function $f(x,y)$ is equivalent to a polar function $g(r,\theta)$. If we wish to integrate $f$ over the region $R$, which is such that $r_1(\theta)\leq r\leq r_2(\theta)$ and $a\leq \theta\leq b$:
\begin{equation*}
    \iint_R f(x,y) \,\mathrm{d}A
	=\int_{a}^{b}\int_{r_1(\theta)}^{r_2(\theta)}g(r,\theta)\, r\,\mathrm{d}r\,\mathrm{d}\theta
\end{equation*}
Note that when converting to polar coordinates, $\mathrm{d}A=r \,\mathrm{d}r \,\mathrm{d}\theta=r \,\mathrm{d}\theta \,\mathrm{d}r$.

\subsection{Change of Coordinates}

\begin{equation*}
    \iint_R f(x,y) \,\mathrm{d}A
	=\iint_R f(\mathbf G(u,v)) \det(\mathbf J_\mathbf G) \,\mathrm{d}\overline A
\end{equation*}
where $\mathbf G:\mathbb R^2\to\mathbb R^2$ applies a change of coordinates:
\begin{equation*}
	(x,y)=\mathbf G(u,v)
\end{equation*}
In other words, where the function $\mathbf G$ maps the new coordinate system $(u,v)$ onto the existing $(x,y)$ system.

To apply the new coordinate system in an integral, the area differential $\mathrm{d}A$ must be scaled by the \textbf{Jacobian determinant} of $\mathbf G$, which is the determinant of the Jacobian matrix. If $\mathbf G(u,v)=(g_1(u,v), g_2(u,v))$:
\begin{equation*}
    \mathbf J_\mathbf G
	=\left[\begin{matrix}
		\displaystyle \diffp{g_1}{u} &
		\displaystyle \diffp{g_1}{v} \\[10pt]
		\displaystyle \diffp{g_2}{u} &
		\displaystyle \diffp{g_2}{v}
	\end{matrix}\right]
	\qquad
    \det(\mathbf J_\mathbf G)
	=\left|\begin{matrix}
		\displaystyle \diffp{g_1}{u} &
		\displaystyle \diffp{g_1}{v} \\[10pt]
		\displaystyle \diffp{g_2}{u} &
	...
	\end{matrix}\right|
\end{equation*}
The area differential for the $(x,y)$ coordinate system $\mathrm{d}A$ in terms of the new coordinate system $\mathrm{d}\overline A$ is:
\begin{equation*}
	\mathrm{d}A 
	=\det(\mathrm J_\mathrm G) \,\mathrm{d}\overline A
\end{equation*}
...
\end{align*}
The Jacobian of $(x,y,z)$ in terms of the spherical coordinates is:
\begin{equation*}
	\mathbf J
	=\begin{vmatrix}
		\nabla x \\
	...
	=\rho^2\sin\varphi
\end{equation*}
To convert an integral in cartesian coordinates to one in spherical coordinates:
\begin{equation*}
    \iiint_R f(x,y,z) \,\mathrm{d}V
	=\iiint_R f(\rho, \theta, \varphi)(\rho^2\sin\varphi) \,\mathrm{d}r \,\mathrm{d}\theta \,\mathrm{d}\varphi
\end{equation*}
Alternatively:
\begin{equation*}
    \mathrm{d}V=\rho^2\sin\varphi\,\mathrm{d}r\,\mathrm{d}\theta\,\mathrm{d}\varphi
\end{equation*}
To integrate over a full sphere, use $0\leq \theta\leq 2\pi, 0\leq \varphi\leq \pi$. Volume is counted twice if $0\leq\varphi\leq 2\pi$.
...
\end{equation*}
is independent of path if there exists a function $f$ such that $\nabla f=(M,N)$. Then the integral on any path from $(x_1,y_1)$ to $(x_2,y_2)$ equals:
\begin{equation*}
    \int_C (M \,\mathrm{d}x+N \,\mathrm{d}y)
	=f(x_2,y_2)-f(x_1,y_1)
\end{equation*}
More generally, if $\nabla f=(M_1,\ldots, M_n)$, then the integral on any path from $\mathbf x_1$ to $\mathbf x_2$ is:
\begin{equation*}
    \int_C \sum_i M_i \,\mathrm{d}x_i
	=f(\mathbf x_2)-f(\mathbf x_1)
\end{equation*}
A line integral is often evaluated along a path given by $r:\mathbb R\to\mathbb R^n$, where the parameter of $r$ can be thought to represent ``time.'' The differential is in terms of the arc length parameter:
\begin{equation*}
    \mathrm{d}s=\lVert r'(t) \rVert \,\mathrm{d}t
\end{equation*}
The line integral:
\begin{equation*}
    \int_C f(x,y) \,\mathrm{d}s
\end{equation*}
represents the integral of the function $f$ over the curve $C$, which in this case is given by $r(t)$ over $a\leq t\leq b$. If you pulled the curve over which the integral is evaluated onto a plane, this is the signed area under/above $f$.

If $r(t)=(x(t), y(t))$, the line integral over the path given by $r(t)$ where $a\leq t\leq b$ is:
\begin{align*}
    \int_C f(x,y) \,\mathrm{d}s
	&=\int_C f(x,y) \lVert r'(t) \rVert dt \\
	&=\int_a^b f(x(t),y(t)) \lVert (x'(t), y'(t)) \rVert \mathrm{d}t
\end{align*}

\section{Vector Calculus}

A vector field is a function $F:\mathbb R^n\to\mathbb R^n$.

\subsection{Green's Theorem}

Let $F:\mathbb R^2\to\mathbb R^2$ be a vector field where $F(x,y)=(M(x,y),N(x,y))$. The scalar curl of $F$ is:
\begin{equation*}
	\diffp Nx-\diffp My
\end{equation*}
Green's theorem states that the line integral over a closed curve $C$ equals the integral of the scalar curls at all points inside the curve (area integral over $R$). Note that $\mathrm{d}r=(\mathrm{d}x,\mathrm{d}y)$.
\begin{equation*}
    \oint_C F\cdot \mathrm{d}r
	=\oint_C \left( M \,\mathrm{d}x+N \,\mathrm{d}y \right)
	=\iint_R \left( \diffp Nx-\diffp My \right) \mathrm{d}A
\end{equation*}

\subsection{Divergence and Curl}

The divergence of a vector field $F(x,y,z)=(M,N,P)$ is defined as:
\begin{equation*}
	\diverge F
	=\nabla \cdot F
	=\diffp Mx+\diffp Ny+\diffp Pz
\end{equation*}
...
\begin{equation*}
	...
	=\begin{vmatrix}
		\hat i & \hat j & \hat k \\
		\displaystyle\diffp{}x & \displaystyle\diffp{}y & \displaystyle\diffp{}z \\
		M & N & P
	\end{vmatrix}
	=\left( \diffp Py-\diffp Nz, \diffp Mz-\diffp Px, \diffp Nx-\diffp My \right)
\end{equation*}
Note that $\diverge F$ is a scalar while $\curl F$ is a vector in $\mathbb R^3$.

\subsection{Stokes' Theorem}

The line integral of $F(x,y,z)=(M,N,P)$ over a closed curve $C$ equals the dot product of $\curl F$ and a unit vector $n$ over the area enclosed by $C$. $n$ is a unit vector oriented in a right hand fashion to the direction of travel defined by $C$, and is orthogonal to the surface created by $F$.
\begin{equation*}
    \oint_C F\cdot \mathrm{d}r
	=\iint_R (\curl F)\cdot n \,\mathrm{d}S
\end{equation*}
where $\mathrm{d}S$ is the surface area differential. If the surface over which the right hand side is calculated can be expressed as $z=g(x,y)$:
\begin{equation*}
    \mathrm{d}S=\sqrt{(g_x)^2+(g_y)^2+1}\,\mathrm{d}A
\end{equation*}

\subsection{Divergence Theorem}

\textbf{In two dimensions.} Let $F(x,y)=(M,N)$ be a vector field, and let $C$ be a closed loop. Let $n$ represent the unit vector orthogonal to $C$ at a certain point (the current location of the integral) and facing out of the curve.
\begin{equation*}
    \oint_C F\cdot n \,\mathrm{d}s
	=\iint_R \left( \diffp Mx+\diffp Ny \right)\mathrm{d}A
	=\iint_R \diverge F \,\mathrm{d}A
\end{equation*}
where $\mathrm{d}s$ is the arc length parameter. If $C$ is defined by a vector function $r(t)$ where $r:\mathbb R\to\mathbb R^2$, $\mathrm{d}s=\lVert r'(t) \rVert \,\mathrm{d}t$. $R$ is the area enclosed by the curve $C$. For the theorem to apply, $\diverge F$ must be defined at all points in $R$.

This value is also known as the \textbf{flux}.

\textbf{In three dimensions.} Let $F(x,y,z)=(M,N,P)$. The flux over a boundary surface $S$ where $n$ is the outward pointing unit vector from the surface $S$ is:
\begin{equation*}
    \oiint_S F\cdot n \,\mathrm{d}S
	=\iiint_Q \diverge F \,\mathrm{d}V
\end{equation*}
where $Q$ is the volume enclosed inside the surface $S$ and $\mathrm{d}S$ is the surface area differential.
